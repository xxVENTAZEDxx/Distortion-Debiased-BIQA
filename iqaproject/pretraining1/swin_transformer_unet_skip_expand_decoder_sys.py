...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=(384,384), patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=(384,384), patch_size=4, in_chans=3, embed_dim=96, depths=[2, 2, 2, 2], depths_decoder=[1, 2, 2, 2], num_heads=[3, 6, 12, 24],
                 window_size=12, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, final_upsample="expand_first", classes=125, **kwargs):
        super().__init__()
    ...
    def forward(self, x):
        ...
        logits = self.classification(torch.mean(x, dim=1))
        # print(x.shape,x_downsample[0].shape,x_downsample[1].shape,x_downsample[2].shape,x_downsample[3].shape)
...
if __name__ == '__main__':
    ...
    model = SwinTransformerSys(img_size=(384,384),patch_size=4, window_size=12)
    out,logits = model(x)
    ...
    flops = model.flops()
    flops_g = flops / 1e9
    print(f'the number of FLOPs: {flops_g:.2f}G')